...
local function math_bound(value,min,max)
  if value<min then return min end
  if value>max then return max end
  return value
end

local function math_cycle(value,n)
  while value>n do value=value-n end
  while value<1 do value=value+n end
  return value
end

if rawget(math, "bound") == nil then
  rawset(math, "bound", math_bound)
end
if rawget(math, "cycle") == nil then
  rawset(math, "cycle", math_cycle)
end

if rawget(table, "clear") == nil then
  rawset(table, "clear", function(tbl)
