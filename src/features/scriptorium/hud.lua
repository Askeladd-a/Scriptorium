...
function Hud.new(deps)
    ...
    function methods.drawMarginNotes(self, page, high_contrast)
        ...
        local line_h = clamp(math.floor(available_h / #lines), RuntimeUI.sized(13), RuntimeUI.sized(18))
        local body_font = get_font(clamp(line_h - RuntimeUI.sized(4), 10, 12), false)
        ...
        for i, icon in ipairs(icons) do
            ...
            love.graphics.setColor(0.34, 0.25, 0.17, 1)
            love.graphics.print(icon.short .. ":" .. icon.value, ix + RuntimeUI.sized(11), y + RuntimeUI.sized(1))
        end
    end
    ...
    function methods.drawStopPushControls(self, high_contrast)
        ...
        if placing_mode then
            ...
            local function draw_action(rect, title, subtitle, base_color, line_color, enabled, title_px, subtitle_px)
                ...
                local title_font = get_font(title_px or 15, true)
                local subtitle_font = get_font(subtitle_px or 10, false)
            ...
            local title_font = get_font(20, true)
            local subtitle_font = get_font(11, false)
