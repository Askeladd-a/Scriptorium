...
function BLOOM.Init()
	...
	ShaderCurve = love.graphics.newShader(
		"shader/profile_curve.FRG.glsl",
		"shader/copy.VTX.glsl",
		{
			debugname = "Shader.Curve"
		}
	)
end
...
function BLOOM.Resize( NewWidth, NewHeight )
	...
	Textures.Curve = love.graphics.newCanvas(
		512, 512,
		{
			type = "2d",
			-- format = "srgba8",
			format = "rgba16f",
			debugname = "Texture.Curve"
		}
	)
end

function BLOOM.Draw( InputTexture, UseDithering )
	GL.PushEvent( "Bloom" )
	ShaderDownsample:send( "WeightMode", BLOOM.WeightMode )
	...
	BLOOM.Composite( InputTexture, UseDithering )
	BLOOM.Curve()

	-- Return composited image (scene + bloom)
	return Textures.Composite, Textures.Curve
end
...
function BLOOM.BlurDownsample( InputTexture, Buffers, Shader )
	...
	for i=1, #Buffers do
		...
		-- otherwise will sample too far creating a square pattern bug
...
function BLOOM.Composite( InputTexture, UseDithering )
	...
	ShaderTonemap:send( "UseDithering", BoolToInt[UseDithering] )
	ShaderTonemap:send( "Random", love.math.random() )
...
function BLOOM.Curve()
	GL.PushEvent( "Bloom curve" )

	-- Generate the curve texture
	love.graphics.setShader( ShaderCurve )
	love.graphics.setCanvas( Textures.Curve )

	ShaderCurve:send( "TextureBuffer", Textures.Composite )
	ShaderCurve:send( "ResolutionSize",
		{
			Textures.Curve:getWidth(),
			Textures.Curve:getHeight()
		}
	)

	COMMON.DrawFullscreenTriangle()

	-- Draw the result over the composite result
	love.graphics.setShader()
	love.graphics.setCanvas( Textures.Composite )
	love.graphics.setBlendMode( "alpha", "alphamultiply" )

	local Mesh = COMMON.GetQuadMesh()
	Mesh:setTexture( Textures.Curve )

	local Width, Height = Textures.Composite:getDimensions()

	love.graphics.draw(
		Mesh,
		Width - 512 - 8, -- Extra 8 pixels padding
		math.floor( (Height - 512) * 0.5 ),
		0,
		512, 512
	)
